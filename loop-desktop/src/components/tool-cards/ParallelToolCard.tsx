    ...
    <div className="mt-3 rounded-lg border border-blue-500/30 bg-blue-500/5 p-3 text-xs">
      ...
      <div className="space-y-1.5">
        {payload.results.slice(0, 8).map((item, index) => (
          <div key={`${item.name}:${index}`} className="rounded border border-neutral-700 bg-neutral-900/50 px-2 py-1.5">
            <div className="flex items-center justify-between gap-2">
              <span className="font-mono text-[11px] text-neutral-100">{item.name}</span>
      ...
      {payload.results.length > 8 ? (
        <p className="mt-2 text-[11px] text-neutral-400">...and {payload.results.length - 8} more</p>
      ) : null}
    </div>
  );
});
